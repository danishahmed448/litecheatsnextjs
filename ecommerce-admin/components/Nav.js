...
const Nav = ({ show, setShowNav }) => {
  ...
  return (
    <aside
      className={`text-gray-500 fixed w-full bg-white h-full top-0 ${
        show ? 'left-0 ' : '-left-full'
      } md:sticky md:w-auto transition-all md:flex-1 md:basis-1/3 md:max-w-sm h-screen`}
    >
      <div className='flex justify-between items-center bg-primary p-4 text-white'>
        <div className=''>
          <Logo />
        </div>
        <button className='md:hidden' onClick={() => setShowNav(false)}>
          <svg
